...
- `create_event` smart routing — requests that omit `calendar_id` can auto-route by env-configured `MORGEN_ACCOUNT_ROUTES` rules using participant domains and title/description keywords, with `MORGEN_DEFAULT_ACCOUNT`/default writable calendar as fallback. Override with explicit `account` set to a configured route name — customized from upstream v0.1.3 routing.
